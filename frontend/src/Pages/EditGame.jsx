...
import Ajv from 'ajv';
import quizSchema from '../2.5.json'
  ...
  function handleFileUpload (event) {
    ...
    reader.onload = () => {
      ...
      if (!isValid) {
        ...
        // Need to assign the correct data file, ie check the name of the quiz and question id
        // This will prevent overlapping quizzes then we merge result onto quizQuestion array
        if (quizData.length !== 0) {
          if (quizData.name === jsonData.name) {
            setId(id + 1);
            jsonData.questions.id = id.toString();
            const object = {
              id: jsonData.questions.id,
              title: jsonData.questions.title,
              type: jsonData.questions.type,
              time: jsonData.questions.time,
              points: jsonData.questions.points,
              url: jsonData.questions.url,
              answer: jsonData.questions.answer,
              solution: jsonData.questions.solution
            }
            setQuizQuestions([...quizQuestions, { object }])
            React.useEffect(() => {
              makeRequest(`/admin/quiz/${params.gameID}`, 'PUT', {
                name: jsonData.name,
